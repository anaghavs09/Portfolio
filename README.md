# 🌐 Anagha Veena Sanjeev - Portfolio

[![Portfolio Live](https://img.shields.io/badge/Portfolio-Live-success?style=for-the-badge)](https://anaghavs09.github.io/portfolio/)
[![GitHub](https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github)](https://github.com/anaghavs09)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/anagha-v-s)

---

## 👋 Welcome to My Portfolio Repository

This is the source code for my personal portfolio website showcasing my work as a **Data Analytics Engineer**.

### 🚀 **[View Live Portfolio →](https://anaghavs09.github.io/portfolio/)**

---

## 💼 About Me

I'm a **Data Analytics Engineer** specializing in:
- 📊 Building scalable data pipelines (dbt, AWS, Airflow)
- 📈 Creating analytics solutions (Tableau, Power BI, SQL)
- 🔍 Solving business problems with data-driven insights
- ⚡ Optimizing data workflows for performance and quality

**Current Status:** MS in Data Analytics Engineering @ Northeastern University | Graduating December 2025

---

## 🛠️ Portfolio Tech Stack

**Built with:**
- HTML5, CSS3, JavaScript
- Responsive design (mobile-friendly)
- Smooth scroll animations
- Font Awesome icons
- GitHub Pages hosting

**Portfolio Features:**
- ✨ Clean, modern design
- 📱 Fully responsive
- 🎨 Smooth animations
- 🚀 Fast loading
- 📊 Project showcase with links to GitHub repos
- 💼 Professional experience timeline
- 🎯 Skills & competencies overview

---

## 📁 Repository Structure
```
portfolio/
├── index.html          # Main HTML file
├── style.css           # Styling and animations
├── script.js           # Interactive features
├── README.md           # This file
```

---

## 🎯 Featured Projects

My portfolio showcases projects in:
- 🤖 **AI/ML Applications:** RebalanceAI (LangChain portfolio analyzer)
- 📊 **Business Analytics:** Customer segmentation, crime forecasting
- 🔍 **Text Analytics:** NLP for recruitment, aviation communications
- 💡 **Data Solutions:** Fraud detection, healthcare analytics

**[Explore all projects →](https://github.com/anaghavs09?tab=repositories)**

---

## 📫 Contact

- **Email:** sanjeev.an@northeastern.edu
- **LinkedIn:** [linkedin.com/in/anagha-v-s](https://www.linkedin.com/in/anagha-v-s)
- **GitHub:** [github.com/anaghavs09](https://github.com/anaghavs09)

---

## 🎓 Currently Seeking

**Full-time roles starting December 2025:**
- Data Engineer
- Analytics Engineer  
- Business Intelligence Engineer
- Data Analyst

**Location:** Boston, MA | Open to relocation

---

## 📝 License

© 2025 Anagha Veena Sanjeev. All rights reserved.

---

<div align="center">

### ⭐ If you found my work interesting, feel free to connect!

**[View Portfolio](https://anaghavs09.github.io/portfolio/)** • **[Connect on LinkedIn](https://www.linkedin.com/in/anagha-v-s)** • **[Email Me](mailto:sanjeev.an@northeastern.edu)**

</div>
